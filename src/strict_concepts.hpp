...
template<typename T>
concept LinearIteratorType = BaseOf<LinearIterator, T> || BaseOf<LinearIterator, typename T::iterator_type>;
...
class Base2D : protected Base{};
template<typename T> concept TwoDimBaseType = BaseOf<Base2D, T>;
...
// NOTE: implementation will change if 3D array is added

namespace internal {

template<BaseType T1, BaseType T2>
auto same_dim = []() consteval -> bool {
   return !(OneDimBaseType<T1> ^ OneDimBaseType<T2>);
};

}

template<typename T1, typename T2>
concept SameDim = internal::same_dim<T1, T2>();
